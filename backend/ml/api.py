"""
FastAPI endpoint for Land Health Prediction.
"""
...
def calculate_confidence(score: float, ndvi: float) -> float:
    ...
    Higher confidence when NDVI clearly indicates health status.
    """
...
@router.get("/predict", response_model=HealthPredictionResponse)
async def predict(
    ...
    soil_pH: float = Query(..., ge=0, le=14, alias="soil_ph", description="Soil pH value"),
    ...
    - **ndvi_mean**: Optional local window mean NDVI
    - **ndvi_std**: Optional local window standard deviation
...
async def predict_post(request: HealthPredictionRequest) -> HealthPredictionResponse:
    """
    Predict land health score from environmental features (POST version).
    """
...
async def model_info() -> dict:
    ...
    if not MODEL_PATH.exists():
        return {
            "status": "not_trained",
            "message": "Model has not been trained yet. Run train.py first."
        }
    ...
    return {
        ...
        "feature_importances": dict(zip(
            ['ndvi', 'ndvi_mean', 'ndvi_std', 'rainfall', 'soil_pH', 'temperature'],
